...
filtered_data_pc <- current_data[current_data$Machine == 1 & current_data$Temperature == 338 & current_data$Pressure == 200, ]
...
if (length(data_for_pc) > 0) {
  ...
  png('media/plots/process_capability_partresistance_m1_t338_p200.png', width=1000, height=600)
  process.capability(qcc_obj, spec.limits=c(LSL, USL), target=Target,
                       nsigmas=3)
  dev.off()
  
  cat('\n--- Process Capability Metrics for PartResistance (M:1, T:338, P:200) ---\n')
  ...
  cat('Not enough valid numeric data for Process Capability for PartResistance (M:1, T:338, P:200).\n')
}
