# R Code for Process Capability Analysis for PartResistance
library(qcc)
current_data <- get("X002..2.")
filtered_data_pc <- current_data[current_data$Machine == 3 & current_data$Temperature == 338 & current_data$Pressure == 200, ]
data_for_pc <- na.omit(as.numeric(filtered_data_pc[['PartResistance']]))
if (length(data_for_pc) > 0) {
  # !!! IMPORTANT: Replace these with actual engineering specifications !!!
  LSL <- 4 # Lower Specification Limit
  USL <- 7 # Upper Specification Limit
  ...
  png('media/plots/process_capability_partresistance_m3_t338_p200.png', width=1000, height=600)
  process.capability(qcc_obj, spec.limits=c(LSL, USL), target=Target,
                       nsigmas=3)
  dev.off()
  
  cat('\n--- Process Capability Metrics for PartResistance (M:3, T:338, P:200) ---\n')
  ...
  cat('Not enough valid numeric data for Process Capability for PartResistance (M:3, T:338, P:200).\n')
}
