...
if (length(data_for_qcc) >= 2) {
  ...
  plot(q, 
       title=paste0('Individual Control Chart for PartResistance (M:3, T:373, P:100)'),
       cex.main=1.5, cex.lab=1.2, cex.axis=1.0, 
       col.points='black', col.lines='blue', col.limits='red', 
       lwd=2, restore.par=FALSE)
  dev.off()
} else {
  cat('Not enough valid numeric data for Control Chart for PartResistance (M:3, T:373, P:100).\n')
}
