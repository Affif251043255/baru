...
filtered_data_pc <- current_data[current_data$Machine == 3 & current_data$Temperature == 303 & current_data$Pressure == 100, ]
...
if (length(data_for_pc) > 0) {
  ...
  png('media/plots/process_capability_partresistance_m3_t303_p100.png', width=1000, height=600)
  process.capability(qcc_obj, spec.limits=c(LSL, USL), target=Target,
                       nsigmas=3)
  dev.off()
  
  cat('\n--- Process Capability Metrics for PartResistance (M:3, T:303, P:100) ---\n')
  ...
  cat('Not enough valid numeric data for Process Capability for PartResistance (M:3, T:303, P:100).\n')
}
