...
filtered_data_cc <- current_data[current_data$Machine == 2 & current_data$Temperature == 338 & current_data$Pressure == 300, ]
...
if (length(data_for_qcc) >= 2) {
  ...
  png('media/plots/control_chart_partresistance_m2_t338_p300.png', width=1000, height=600)
  plot(q, 
       title=paste0('Individual Control Chart for PartResistance (M:2, T:338, P:300)'),
       cex.main=1.5, cex.lab=1.2, cex.axis=1.0, 
       col.points='black', col.lines='blue', col.limits='red', 
       lwd=2, restore.par=FALSE)
  dev.off()
} else {
  cat('Not enough valid numeric data for Control Chart for PartResistance (M:2, T:338, P:300).\n')
}
